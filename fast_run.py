...
# NOTE: leave this commented on single-GPU-visible machines (like your current setup)
...
if task_name == 'laptop14':
    train_batch_size = 32
elif task_name in ('rest_total', 'rest14', 'rest15', 'rest16'):
    train_batch_size = 16
else:
...
for run_id, seed in enumerate(seed_numbers):
    command = (
        "python main.py --model_type %s --absa_type %s --tfm_mode %s --fix_tfm %s "
